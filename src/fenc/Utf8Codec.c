 ...
 */
static
int Utf8Codec_getNumUtf8Bytes(const int32 ucs4) {
    if (ucs4 < 0) {
        return 7;
    } else if (ucs4 <= 0x7F) {
        return 1;
...
static
int Utf8Codec_encode1unit(int32 ucs4, byte * utf8, int start) {
    int n = Utf8Codec_getNumUtf8Bytes(ucs4);
    if (n == 1) {
        utf8[start++] = ucs4;
        return 1;
    }
...
int Utf8Codec_encode(int32 * ucs4, int i, int j, byte * utf8, int start) {
    ...
    int k = start;
    while (i < j) {
        int numProduced = Utf8Codec_encode1unit(ucs4[i], utf8, k);
        i++;
        k += numProduced;
    }
    return k - start;
...
static
boolean Utf8Codec_isUtf8Head(const byte aByte) {
    return (aByte & 0xC0) != 0x80 && (aByte & 0xFE) != 0xFE;
}

static
int Utf8Codec_getNumUtf8BytesByUtf8Head(const byte headByte) {
    assert(Utf8Codec_isUtf8Head(headByte));
...
static
int Utf8Codec_decode1unit(byte * utf8, int i, int32 * outUcs4) {
    assert(utf8 != NULL);
    assert(i >= 0);
    assert(outUcs4 != NULL);

    int n = Utf8Codec_getNumUtf8BytesByUtf8Head(utf8[i]);
    switch (n) {
        case 0:
            break;
        case 1:
...
int Utf8Codec_decode(byte * utf8, int i, int j, int32 * ucs4, int start) {
    ...
    int k = start;
    while (i < j) {
        int numConsumed = Utf8Codec_decode1unit(utf8, i, ucs4 + k);
        i += numConsumed;
        k++;
    }
    return k - start;
